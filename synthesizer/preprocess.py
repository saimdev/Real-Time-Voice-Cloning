...
def preprocess_dataset(datasets_root: Path, out_dir: Path, n_processes: int, skip_existing: bool, hparams,
    ...
    metadata = []  # Collect metadata for all speakers
    for speaker_metadata in tqdm(job, datasets_name, len(speaker_dirs), unit="speakers"):
        metadata.extend(speaker_metadata)
...
def preprocess_speaker(speaker_dir, out_dir: Path, skip_existing: bool, hparams, no_alignments: bool):
    ...
    for book_dir in speaker_dir.glob("*"):
        ...
        if no_alignments:
            ...
            wav_fpaths = list(book_dir.glob("*.flac"))
            ...
            for wav_fpath in wav_fpaths:
                ...
                final_folder_name = book_dir.name
                text_fpath = book_dir.joinpath(final_folder_name + ".trans.txt")
            ...
            # Use .flac files directly
            print("TESTING")
            wav_fpaths = list(book_dir.glob("*.flac"))
            print("Wav fpaths:", wav_fpaths)
            for wav_fpath in wav_fpaths:
                # Load the audio waveform
                print("Wav fpath:", wav_fpath)
                wav, _ = librosa.load(str(wav_fpath), hparams.sample_rate)
                if hparams.rescale:
                    wav = wav / np.abs(wav).max() * hparams.rescaling_max

                # Get the corresponding text
                text_fpath = book_dir.joinpath(wav_fpath.stem + ".trans.txt")
                with text_fpath.open("r", encoding="utf-8") as text_file:
                    text = text_file.read().strip().split(maxsplit=1)[1]
...
def process_utterance(wav: np.ndarray, text: str, out_dir: Path, basename: str,
    ...
    # print(f"Processing: {basename} - Text: {text}")
